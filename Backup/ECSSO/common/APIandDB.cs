    ...
    public class APIandDB
    {
            ...
            //try
            {
                if (MemID != "")
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(setting))
                    {
                        ...
                        Random r = new Random();
                        pwd += r.Next(1, 9999999).ToString();   //加上一些亂數yo~

                        cmd.Parameters.Add(new SqlParameter("@mem_id", MemID));
                        cmd.Parameters.Add(new SqlParameter("@id", OAuthemail));
                        cmd.Parameters.Add(new SqlParameter("@pwd", pwd));
                        cmd.Parameters.Add(new SqlParameter("@ch_name", ""));
        ...
        private void chToY(string MemID)
        {
            ...
            using (SqlConnection conn = new SqlConnection(setting))
            {
                ...
                SqlDataReader reader = getbonus.ExecuteReader();
                ...
                if (reader.HasRows)
                {
                    try
                    {
                        ...
                        reader.Close(); //記得要先把reader or command關閉,這個conn才可以再繼續下一個command
        ...
        public string getEmail(string siteid, string OAuthSnAndId)
        {
            ...
            using (SqlConnection conn = new SqlConnection(setting))
            {
                ...
                //搜尋看有沒有這個會員的SQL語法
                String Str_sql = "select id from Cust where SnAndId=@OAuthSnAndId and chk='Y'";
                SqlCommand cmd = new SqlCommand(Str_sql, conn);
