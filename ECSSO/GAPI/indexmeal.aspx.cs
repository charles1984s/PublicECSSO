...
{
    public partial class indexmeal : System.Web.UI.Page
    {
        //該OAuth app 的基本資料
        string RedirectURIs = ConfigurationManager.AppSettings.Get("GAPI_RedirectURIs_Meal");
        string Client_ID = ConfigurationManager.AppSettings.Get("GAPI_Client_ID_Meal");
        string Client_secret = ConfigurationManager.AppSettings.Get("GAPI_Client_secret_Meal");
                        ...
                        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["sqlDB"].ToString()))
                        {
                            ...
                            try
                            {
                                if (reader.HasRows)
                                {
                                    while (reader.Read())
                                    {
                                        Session["weburl"] = reader["web_url"].ToString();
                                        Session["returnurl"] = Session["weburl"] + "?" + Session["returnurl"].ToString().Split('?')[1];
                                    }
                                }
                            }
                            catch
