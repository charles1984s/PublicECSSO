...
namespace ECSSO
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["sqlDB"].ToString()))
                {
                    ...
                    try
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if (reader["web_url"].ToString().IndexOf("http") >= 0) this.weburl.Value = reader["web_url"].ToString();
                                else this.weburl.Value = String.Format("http://{0}",reader["web_url"].ToString());
                                this.returnurl.Value = String.Format("{0}/{1}/index.asp?{2}",
                                        this.weburl.Value, getstr.GetLanString(str_language), this.returnurl.Value.Split('?')[1]);
                            }
                        }
                    }
                    catch
                ...
                if (GS.MD5Check(this.siteid.Value, CheckM))
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand("select dbname,dbusername,CONVERT(nvarchar(50), dbpassword) dbpassword,web_url,fid from cocker_cust where id=@id", conn);
                        cmd.Parameters.Add(new SqlParameter("@id", this.siteid.Value));
                        SqlDataReader reader = cmd.ExecuteReader();
                        try
                        {
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    ...
                                    {
                                        if (this.returnurl.Value.IndexOf("http")>=0) this.returnurl.Value = reader["web_url"].ToString();
                                        else this.returnurl.Value = "http://" + reader["web_url"].ToString();
                                    }
                                    if (this.weburl.Value == "")
                                    {
                                        if (this.returnurl.Value.IndexOf("http") >= 0) this.weburl.Value = reader["web_url"].ToString();
                                        else this.weburl.Value = "http://" + reader["web_url"].ToString();
                            ...
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    ...
                                    if (reader[1].ToString() == "N")
                                    {
                                        ...
                                        other_title.Visible = false;
                                        fbErrorInfo.Visible = false;
                                        if (reader[2].ToString() == "N")
                                        {
                                            ...
                                            AddMemberRow.Visible = false;
                                            MemberLoginRow.Attributes.Remove("class");
                                            MemberLoginRow.Attributes.Add("class", "col-md-12");
                                        }
                                        else
                                        {
                            ...
                            if (reader.HasRows)
                            {
                                ...
                                else
                                {
                                    ...
                                    using (SqlConnection conn1 = new SqlConnection(setting))
                                    {
                                        ...
                                        reader.Read();
                                        cmd1.Parameters.Add(new SqlParameter("@memid", reader["mem_id"].ToString()));
                                                ...
                                                while (reader1.Read())
                                                {
                                                    switch (reader1["cid"].ToString())
                                                    {
                                                        case "2":
                                                            ...
                                                            scaned += "," + "63124";
                                                            break;
                                                        case "3": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "2350";
                                                            break;
                                                        case "4": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "4544";
                                                            break;
                                                        case "5": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "7872";
                                                            break;
                                                        case "6": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "60813";
                                                            break;
                                                        case "7": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "2348";
                                                            break;
                                                        case "8": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "13964";
                                                            break;
                                                        case "9": totalPrice += int.Parse(reader1["Price"].ToString());
                                                            scaned += "," + "1728";
                                                            break;
                                                    }
                                                }
                                    ...
                                    {
                                        ////////////////臨時給大仁用的///////////////////
                                        Response.Redirect(this.weburl.Value + "/" + getstr.GetLanString(this.language.Value) + "/log.asp?id=" + this.UserID.Text + "&VerCode=" + this.VerCode.Value + "&tokenid=" + TokenID + "&totalPrice=" + totalPrice + "&scaned=" + scaned + "&ReturnUrl=" + this.returnurl.Value.Replace("&", "////").Split('?')[1]);
                                    ...
                                    {
                                        ////////////////臨時給大仁用的///////////////////
                                        Response.Redirect(this.weburl.Value + "/" + getstr.GetLanString(this.language.Value) + "/log.asp?id=" + this.UserID.Text + "&VerCode=" + this.VerCode.Value + "&tokenid=" + TokenID + "&totalPrice=" + totalPrice + "&scaned=" + scaned + "&ReturnUrl=au_id=a////sub_id=b");
        ...
        {
            Session["siteid"] = this.siteid.Value;
            Session["weburl"] = this.weburl.Value;
            Session["language"] = this.language.Value;
            Session["VerCode"] = this.VerCode.Value;
            Session["returnurl"] = this.returnurl.Value;
            
            Response.Redirect("/api/fblogin.ashx");
        ...
        protected void LinkButton4_Click1(object sender, EventArgs e)
        {
            ...
            if (!this.CheckEmail.Visible && !this.CheckPwd.Visible && !this.CheckPwd2.Visible && !this.CheckName.Visible && !this.CheckService.Visible)
            {
                if (Session["CheckCode"] != null && String.Compare(Session["CheckCode"].ToString(), this.TextBox2.Text, true) == 0)
                {
                    ...
                    if (MemID != "")
                    {
                        ...
                        {
                            if (this.CheckBox2.Checked)     //同意加入易售聯網會員
                            {
                                Library.Member.Data member = new Library.Member.Data
                                {
                                    ID = this.Email.Text,
                                    Pwd = this.Pwd.Text,
                                    ChName = this.Name.Text,
                                    Sex = this.Sex.SelectedItem.Value,
                                ...
                                string param = "CheckM=" + getstr.MD5Endode(ConfigurationManager.AppSettings.Get("O2OID") + ConfigurationManager.AppSettings.Get("O2OName")) + "&SiteID=" + ConfigurationManager.AppSettings.Get("O2OID") + "&Items=" + System.Web.HttpUtility.UrlEncode(JsonConvert.SerializeObject(member));

                                getstr.SendForm(ConfigurationManager.AppSettings.Get("O2OURL"), param);
                        ...
                        if (CertificationURL == "Y")
                        {
                            ...
                            Mail_Cont += "    <a href='" + this.weburl.Value + "/" + getstr.GetLanString(this.language.Value) + "/checkm.asp?mem_id=" + MemID + "'>" + this.weburl.Value + "/" + getstr.GetLanString(this.language.Value) + "/checkm.asp?mem_id=" + MemID + "</a><br>";
