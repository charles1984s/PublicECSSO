...
namespace ECSSO.api
{
    ...
    /// getEvent 的摘要描述
    /// </summary>
    public class getEvent : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int statement = 0;
            string returnMsg = "Something has wrong!!", tableName = "Active";
            try
            {
                    ...
                    case 0:
                        {
                            ...
                            Events items = JsonConvert.DeserializeObject<Events>(context.Request.Params["Items"]);
                            ...
                            if (items.Range != null)
                            {
                                ...
                                {
                                    minValue = (Int32.TryParse(items.Range.From, out minValue) ? Int32.Parse(items.Range.From) : 1);
                                }
                            ...
                            {
                                string itemString = "";
                                if (Type == "detailsEvents" || Type == "detailsEventsW") itemString = items.ID;
                                else itemString = Items;
                                returnMsg = new CommandSetting().getDataResult(GS.GetIPAddress(), Token, strSqlConnection, Type, Lng, tableName, itemString, items.keyword, minValue, maxValue);
                            }
                            else
                            {
        ...
        private String ErrorMsg(String RspnCode, String RspnMsg, String Setting)
        {
            if (Setting != "")
            {
                InsertLog(Setting, "getEvent error", "", RspnMsg);
            }
        ...
        private void InsertLog(String Setting, String JobName, String JobTitle, String Detail)
        {
            using (SqlConnection conn = new SqlConnection(Setting))
            {
                ...
                cmd.Parameters.Add(new SqlParameter("@prog_name", "取得活動"));
